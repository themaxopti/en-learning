...
type Action = (
  state: WritableDraft<DictionaryState>,
...
export const dictionaryActions: DictionaryActionsType = {
  ...
  addManyWords: (
    ...
    action: PayloadAction<
      { title: string; translate: string; id: number; index: number }[]
    >
  ) => {
  ...
  hideSelectedItems: (state) => {
    const checkedWords = state.words.filter(el => el.checked === true)
