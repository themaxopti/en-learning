...
export function* getDictionariesSaga({
  payload: { limit, page },
}: GetDictionariesSagaParam) {
  try {
    yield put(setDictionariesLoading(true))
    const response: GetDictionariesRes = yield dictionaryApi.getDictionaries({
      limit,
      page,
    })
...
export function* createDictionarySaga({
  payload: { title },
}: CreateDictionarySagaParam) {
  try {
    const response: CreateDictionaryRes = yield dictionaryApi.createDictionary({
      title,
    })
...
export function* getWordsSaga({
  payload: { limit, page, dictionaryId },
}: GetWordsSagaParam) {
  try {
    yield put(setWordsLoading(true))
    yield put(setIsDictionaryExist(true))

    const wordsResponse: GetWordsRes = yield dictionaryApi.getWords({
      limit,
      page,
      dictionaryId,
    })
    ...
    if (
      wordsResponse.statusCode === 404 ||
      currentDictionaryResponse.statusCode === 404
    ) {
      yield delay(5000)
...
export function* createWordSaga({
  payload: { title, translate },
}: CreateWordSagaParam) {
  try {
    ...
    const response: CreateWordRes = yield dictionaryApi.createWord({
...
export function* createWordsSaga({ payload }: CreateWordsSagaParam) {
  try {
    ...
    const response: CreateWordsRes = yield dictionaryApi.createWords({
      words: payload.words,
      dictionaryId: dictionary.id,
    })
...
export function* deleteWordSaga({ payload }: DeleteWordSagaParam) {
  try {
    ...
    const response: DeleteWordRes = yield dictionaryApi.deleteWord({
      id: payload.id,
      dictionaryId: dictionary.id,
    })
    if (response.statusCode !== 200) {
...
export function* changeWordsIndexesSaga({}: ChangeWordsIndexesSagaParam) {
  try {
    ...
    const { arrayWithChangedIndexes, changedIndexes } =
      changedIndexesHelper(wordsCopy)
    const response: DeleteWordsRes = yield dictionaryApi.changeWordsIndexes({
      wordsIndexes: changedIndexes,
    })
